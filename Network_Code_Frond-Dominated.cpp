...
int main() {
    ...
    bool FastPlankton = true;

    // All solid tissue values
    float TriradialPIB = 0.264; //Rugoconites
    float FrondPIB = 15.9771; // Aspidella
    float FunisiaPIB = 8.29455;
    float DickinPIB = 0.435424;
    float AuloPIB = 259.18134;

    //Hardcoded per abundances values (not per square metre, since no area is provided; assume 10 square metre for simplicity and order-of-magnitude alignment with other surfaces)
    float TriradialPerSqMAbundance = 0.2; // 0.2
    float FrondPerSqMAbundance = 5.5; // 5.5
    float FunisiaPerSqMAbundance = 0.1; // 0.1
    float DickinPerSqMAbundance = 0.5; // 0.5
    float AuloPerSqMAbundance = 0.1; // 0.1
    ...
    ReplicateNumber++;
    if ((ConsoleOutputs == false) and (ReplicateNumber % 100 == 0)) {
